...
#' @param eps Optional probability floor for Fisher denominators (default 0 = disabled)
#' @return d x d Fisher information matrix for setting a
fisher_info_setting <- function(theta, a, S_ab, c_ab, ab_df, eps = 0) {
  ...
  if (eps > 0) {
    ...
    if (any(!is.finite(p_a)) || any(p_a <= 0)) {
      stop(sprintf("Encountered nonpositive p_{a,b}(theta) at setting a=%d. Set eps>0 explicitly if needed.", a))
    }
    p_use <- p_a
  }
  ...
  Ia <- 0.25 * crossprod(S_a, S_a * (1 / p_use))
...
fisher_info_all_settings <- function(theta, S_ab, c_ab, ab_df, eps = 0) {
...
total_fisher_info <- function(theta, counts, S_ab, c_ab, ab_df, eps = 0) {
  A <- sort(unique(ab_df$a))
  d <- ncol(S_ab)
...
#' @param rho Density matrix (PSD; can be rank-deficient)
#' @param Y Hermitian matrix
#' @param tol Optional eigenvalue floor for Lyapunov inversion (default 0 = disabled)
#' @return X such that Ω_ρ(X) ≈ Y
lyapunov_inv <- function(rho, Y, tol = 0) {
  ...
  U <- eg$vectors
  # Eigenvalues of Hermitian matrix are real for Hermitian rho.
  lam <- Re(eg$values)
  if (any(!is.finite(lam))) {
    stop("Lyapunov inverse failed: rho has non-finite eigenvalues.")
  }
  lam <- pmax(lam, 0)
  if (tol > 0) lam <- pmax(lam, tol)
  ...
  if (any(!is.finite(denom))) {
    stop("Lyapunov inverse failed: non-finite denominator in 2/(lambda_i+lambda_j).")
  }
  Xtilde <- matrix(0+0i, N, N)
  mask <- denom > 0
  Xtilde[mask] <- (2 / denom[mask]) * Ytilde[mask]
...
metric_bures <- function(theta, sigmas, N, tol = 0) {
  rho_raw <- rho_of_theta(theta, sigmas, N)
  rho <- enforce_rho_floor(rho_raw, eta = 0, sigmas = sigmas)$rho_hat
  d <- length(sigmas)
...
#' @param ridge Optional ridge for explicit regularization (default 0 = disabled)
...
proxy_risk <- function(G, J, ridge = 0) {
  ...
  if (is.null(J_inv) || any(!is.finite(J_inv))) return(Inf)
  out <- sum(diag(G %*% J_inv))
